...
export class DashboardComponent {
  ...
  constructor(
    ...
    this.dashboardService
      .getPatientHealthDetails(patientId, organizationCode)
      .then(result => {
        this.patientHealthStats = result;
        this.patientBMI =
          (this.patientHealthStats.weight / this.patientHeight) ^ 2;
      });
    ...
    this.dashboardService.getFoodOfTheMonth().then(foodOfTheMonthId => {
      this.foodOfTheMonth = this.availableFoods.find(
        food => food.id === foodOfTheMonthId
      );
